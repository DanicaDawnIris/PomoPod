...
const HomeScreen = ({ navigation }) => {
  ...
  const handleCheckButton = (taskName) => {
    ...
    // Update completion status based on the task name
    fetch(API_URL, {
      method: 'PUT', // Assuming you use PUT method for updating tasks
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        task_name: taskName,
        completed: updatedTasks.find((task) => task.task_name === taskName)?.completed || false,
      }),
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((json) => {
        if (json.message === 'Task updated') {
          console.log(`Task with task_name ${taskName} updated successfully in the database.`);
        } else {
          console.log(`Task update failed for task_name ${taskName}. Server message: ${json.message}`);
        }
      })
      .catch((error) => console.error(error));
  };
